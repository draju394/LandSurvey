...
{
    public partial class SearchLandRelatedDocument : System.Web.UI.Page
    {
        ...
        protected void SummaryGridBind()
        {
            ...
            dsSummary = dbReportsData.getEightaDetailsByKhataNo(cmbVillage.SelectedValue, txtKhataNo.Text);

            if (dsSummary != null && dsSummary.Tables[0].Rows.Count > 0)
            {

                dsDetails = dbReportsData.getKhatedarNames(txtKhataNo.Text);
                if (dsDetails != null && dsDetails.Tables[0].Rows.Count > 0)
                {
                    DataTable dtDetail = dsDetails.Tables[0];

                    lblKhataNoValue.Text = dtDetail.Rows[0]["khatano"].ToString();
                    lblKhataTypeValue.Text = dtDetail.Rows[0]["khatanamem"].ToString();
                    lblKhatedarValue.Text = dtDetail.Rows[0]["khatedar"].ToString();
                }
