...
namespace LandSurvey.Reports
{
    public partial class GetClassOfLandOccupant : System.Web.UI.Page
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                if (!Page.IsPostBack)
                {
                    ...
                    dsLandClass = dbReportsData.getLandClass();

                    if (dsLandClass != null && dsLandClass.Tables[0].Rows.Count > 0)
                    {
                        DataTable dtLandClass = dsLandClass.Tables[0];
                        cmbLandClass.DataSource = dsLandClass.Tables[0].DefaultView;
                        cmbLandClass.DataBind();
                        cmbLandClass.DataTextField = dsLandClass.Tables[0].Columns["landclassname"].ToString();
                        cmbLandClass.DataValueField = dsLandClass.Tables[0].Columns["landclasscode"].ToString();
                        cmbLandClass.DataBind();
                    }
                }
            }
        ...
        {
            dsSummary = dbReportsData.getClassOfLandOccupant(cmbVillage.SelectedValue, cmbLandClass.SelectedValue);
        ...
        protected void Grid1_ServerPdfExporting(object sender, GridEventArgs e)
        {
            ...
            if (dsSummary.Tables[0].Rows.Count > 0)
            {
                ...
                PdfDocument pdfDocument = exp.Export(Grid1.Model, (IEnumerable)list, "ClassOfLandOccupant.pdf", true, true, "flat-lime", true);
                ...
                pdfDocument.Save("ClassOfLandOccupant.pdf", Response, HttpReadType.Save);
